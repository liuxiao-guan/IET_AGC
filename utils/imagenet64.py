...
class DownsampledImageNet(VisionDataset):
    ...
    def __init__(self, root: str, selected_classes:list,train: bool = True,
        ...
        #     self.targets = new_targets
          # 筛选指定类别的数据
        if selected_classes is not None and num_classes is not None:
            # 仅保留特定类别图像
            self.data = [img for img, label in zip(self.data, self.targets) if label in selected_classes]
            self.targets = [label for label in self.targets if label in selected_classes]
    ...
    def _check_integrity(self) -> bool:
        root = self.root
        for filename, md5 in (self.train_list):
